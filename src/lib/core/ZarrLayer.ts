...
export class ZarrLayerControl implements IControl {
  ...
  private _eventHandlers: Map<ZarrLayerEvent, Set<ZarrLayerEventHandler>> = new Map();
  ...
  private _renderExpanded(): void {
    ...
    selectorInput.value = this._state.selector ? JSON.stringify(this._state.selector) : '';
    selectorInput.addEventListener('input', () => {
      try {
        const parsed = selectorInput.value ? JSON.parse(selectorInput.value) : {};
    ...
    addBtn.className = 'maplibre-gl-zarr-layer-btn maplibre-gl-zarr-layer-btn--primary';
  ...
  private async _addLayer(): Promise<void> {
    ...
    try {
      ...
      const layerOptions = {
        ...
        selector: this._state.selector && Object.keys(this._state.selector).length > 0
