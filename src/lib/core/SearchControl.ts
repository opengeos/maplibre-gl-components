 ...
 */
const DEFAULT_OPTIONS: Required<Omit<SearchControlOptions, 'geocoder'>> & { geocoder?: SearchControlOptions['geocoder'] } = {
  position: 'top-right',
  className: '',
  visible: true,
  collapsed: true,
...
export class SearchControl implements IControl {
  ...
  private _options: Required<Omit<SearchControlOptions, 'geocoder'>> & { geocoder?: SearchControlOptions['geocoder'] };
  ...
  async search(query: string): Promise<void> {
    ...
    try {
      ...
      this._state.loading = false;
      this._state.error = error instanceof Error ? error.message : 'Search failed';
      this._state.results = [];
      this._updateLoadingState();
      this._renderResultsOnly();
    }
  ...
  private _renderResultsOnly(): void {
    ...
    const existingError = this._container.querySelector('.maplibre-gl-search-error');
    if (existingError) {
  ...
  private _updateLoadingState(): void {
    ...
    const collapseBtn = this._inputWrapper.querySelector('.maplibre-gl-search-collapse');
